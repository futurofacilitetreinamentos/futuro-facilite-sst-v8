...
const V8Report=(()=>{
 ...
 function head(title,n,total){return `<div class="pgr-head"><img src="${logo}"><b>${title}</b></div><div class="pgr-foot"><span>(61) 98515-0519 · futurofacilitetreinamentos@gmail.com</span><span>Página ${n} de ${total}</span></div>`}
 ...
 function build(p){
  ...
  const actions=(p.riscos||[]).filter(r=>r.acao);
  ...
  pages.push(`<h1 class="pgr-title">9. Implantação de medidas de controle e EPI</h1><div class="pgr-text">${esc(T.controles)}</div><div class="pgr-text">${esc(T.hierarquia)}</div><h2 class="pgr-sub">9.1. Equipamento de proteção individual</h2><div class="pgr-text">${esc(T.epi)}</div>`);
  actionChunks.forEach((ch,ix)=>{
   pages.push(`<h1 class="pgr-title">10. Plano de ação ${actionChunks.length>1?`(${ix+1}/${actionChunks.length})`:''}</h1>${ix===0?`<div class="pgr-text">O cumprimento das ações é de responsabilidade da organização, observando prazos, metas e prioridades. Inclui as medidas do inventário e, quando o DRPS indicar gravidade alta, as intervenções organizacionais do item 8.</div>`:''}<table class="pgr-table compact"><thead><tr><th>Risco</th><th>GR</th><th>Medida proposta</th><th>Responsável</th><th>Prazo</th><th>Status</th></tr></thead><tbody>${ch.map(r=>`<tr><td>${esc(r.perigo)}</td><td>${esc(E.grNome(E.gr(r.prob,r.sev)))}</td><td>${esc(r.acao)}</td><td>${esc(r.responsavel)}</td><td>${fmt(r.prazo)}</td><td>${esc(r.status||'Pendente')}</td></tr>`).join('')||'<tr><td colspan="6">Nenhuma ação cadastrada no inventário.</td></tr>'}</tbody></table>`);
  });
  pages.push(`<h1 class="pgr-title">11. Considerações finais</h1><div class="pgr-text">${esc(T.final)}</div><h1 class="pgr-title">12. Referências</h1><div class="pgr-text">${esc(T.refs)}</div>`);
  pages.push(respBlock(p,{
   titulo:'13. Responsabilidade técnica',
 ...
 function buildPcmso(p){
  ...
  pages.push(`<h1 class="pgr-title">3. Desenvolvimento</h1><div class="pgr-text">${esc(T.tipos)}</div><h2 class="pgr-sub">3.1 a 3.5. Periodicidade</h2><div class="pgr-text"><b>Admissional:</b> antes de o empregado assumir as atividades.<br><b>Periódico:</b> anual (ou menor, a critério médico) para expostos a riscos classificados no PGR; a cada dois anos para os demais.<br><b>Retorno ao trabalho:</b> após ausência igual ou superior a 30 dias por doença ou acidente.<br><b>Mudança de risco ocupacional:</b> antes da mudança, adequando o controle médico aos novos riscos.<br><b>Demissional:</b> em até 10 dias do término do contrato, podendo ser dispensado se o exame clínico mais recente estiver dentro dos prazos da NR-7 conforme o grau de risco.</div><h2 class="pgr-sub">3.6. Exames complementares</h2><div class="pgr-text">Podem ser realizados outros exames complementares, a critério do médico responsável, desde que relacionados aos riscos ocupacionais classificados no PGR e tecnicamente justificados neste PCMSO.</div>`);
  ...
  pages.push(`<h1 class="pgr-title">9. Anexos — eSocial</h1><div class="pgr-text">${esc(T.esocial)} As informações referentes aos exames médicos constam da tabela n.º 27 do Anexo I dos leiautes do eSocial (Procedimentos Diagnósticos), no portal esocial.gov.br.</div>`);
