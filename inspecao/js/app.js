...
function init(){
 ...
 $('cnpj').addEventListener('input',e=>e.target.value=formatCNPJ(e.target.value));
}
...
function saveReport(){const d=collect();if(!d.condominio)return alert('Informe o nome do condomínio.');currentId=d.id;const existed=!!FFStorage.get(d.id);FFStorage.save(d);$('editingBanner').classList.add('hidden');$('saveBtn').textContent='Salvar';renderHistory();alert(existed?'Alterações salvas com sucesso.':'Relatório salvo neste aparelho.')}
...
function renderHistory(){renderHistoryInto($('history'),FFStorage.list())}
function renderSaved(){const q=$('searchSaved').value.toLowerCase();renderHistoryInto($('savedList'),FFStorage.list().filter(d=>(d.condominio||'').toLowerCase().includes(q)))}
...
function newReport(){if(!confirm('Iniciar nova inspeção? Salve as alterações atuais antes de continuar.'))return;currentId=null;coverPhoto='';photos={};fields.forEach(f=>$(f).value='');$('tecnico').value='Francson Menezes Alves';$('assinaturaTecnico').value='Francson Menezes Alves';$('objetivo').value='Avaliar as condições de segurança e saúde no ambiente de trabalho do condomínio, identificar perigos e recomendar medidas preventivas e corretivas.';$('dataVisita').value=new Date().toISOString().slice(0,10);$('reportNumber').textContent=FFStorage.nextNumber();$('fotoCapaPreview').innerHTML='';$('editingBanner').classList.add('hidden');$('saveBtn').textContent='Salvar';renderChecklist()}
